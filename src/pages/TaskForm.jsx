...
function TaskForm() {
    ...
    const {  data: project, loading: projectLoading } = useFetch(`${import.meta.env.VITE_BACKEND_URL}/projects/${projectId}`);
    ...
    async function addNewTag(e) {
        ...
        try {
            ...
            setNewTagName("");
            // Refresh tags list without full reload
    ...
    async function handleTaskCreation(e) {
        ...
        try {
            ...
            // Reset form
            setTaskName(""); setTeamName(""); setStatus(""); setTimeToComplete(0); setTag([]); setOwnerName([]);
    ...
    return (
        <>
            ...
            <style>{`
                ...
                .secondary-btn {
                    background: rgba(40, 139, 221, 0.15);
                    ...
                    <div className="glass-panel p-3 p-md-4 mt-4 animate-in">
                        ...
                        <div className="card-header bg-transparent border-0 text-center pb-3">
                            <h4 className="mb-0 fw-bold" style={{ 
                                ...
                                {projectLoading ? (
                                    <span className="loading-skeleton rounded-pill" style={{ width: '200px', height: '1.5rem', display: 'inline-block' }}></span>
                                ) : (
                                    `Create New Task for '${project?.project?.name || 'Project'}'`
                                )}
                            </h4>
                            <p className="text-muted mb-0 mt-1 fs-6">Fill in the details to add a new task</p>
                        </div>
                            ...
                            <div className="row g-3 g-md-4">
                                ...
                                <div className="col-lg-3">
                                    <div className="glass-sidebar p-3 h-100">
                                        ...
                                        <Link to={`/projects/${projectId}`} className="sidebar-link text-decoration-none p-3 rounded-3">
                                            <MdKeyboardArrowLeft className="fs-5" />
                                            <span className="fw-medium">Back to Project</span>
                                        </Link>
                                    </div>
                                </div>
                                    ...
                                    <form onSubmit={handleTaskCreation}>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <small className="text-muted d-block mt-1 ps-1">
                                                💡 Hold <kbd className="bg-dark border px-1 rounded">Ctrl</kbd> or <kbd className="bg-dark border px-1 rounded">Cmd</kbd> to select multiple
                                            </small>
                                        </div>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <small className="text-muted d-block mb-2 ps-1">
                                                💡 Hold <kbd className="bg-dark border px-1 rounded">Ctrl</kbd> or <kbd className="bg-dark border px-1 rounded">Cmd</kbd> to select multiple
                                            </small>
                                            {/* Inline Add Tag */}
                                        ...
                                        <div className="d-grid gap-3">
                                            <button
                                                ...
                                                disabled={isSubmitting || !taskName.trim() || !status}
                                            >
                                                {isSubmitting ? (
                                                    <>
                                                        <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
